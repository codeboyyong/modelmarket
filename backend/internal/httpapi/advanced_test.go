...
func TestRefundPayment(t *testing.T) {
	...
	defer cleanup()
	mock.ExpectBegin()
	mock.ExpectQuery("select wallet_id, amount_cents").WithArgs("payment-1").WillReturnRows(sqlmock.NewRows([]string{"wallet_id", "amount_cents", "refunded_amount_cents", "status", "provider", "provider_payment_id"}).AddRow("wallet-1", int64(1000), int64(0), "succeeded", "mock", "fake-1"))
	...
	mock.ExpectExec("insert into user_ledger_transactions").WithArgs(sqlmock.AnyArg(), "wallet-1", int64(-500), sqlmock.AnyArg(), sqlmock.AnyArg()).WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
	recorder := httptest.NewRecorder()
